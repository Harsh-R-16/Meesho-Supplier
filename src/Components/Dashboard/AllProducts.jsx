...
import { MainAllProducts } from "./Styled-Components";
...
export default function AllProducts() {
  ...
  React.useEffect(() => {
    fetch("https://meeshodb.herokuapp.com/api/v1/products")
      ...
      .then((res) => {
        let temp = [];
        let query = localStorage.getItem("seller");
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].soldBy === query) temp.push(res.data[i]);
        }
  ...
  return (
    <MainAllProducts>
      <h1>{localStorage.getItem("seller")}</h1>
      <section id="products">
        {products.map(
          ({ img, name, sprice, aprice, rating, reviews, _id }, index) => (
